...
- **Workflow invariants** — `INVARIANTS.md` documents four enforcement-backed guarantees (required-capability nonskip, monotone re-detect, authorized reduction only, absolute floor); regenerate `CAPABILITIES.md` via `python3 "${CURSOR_PLUGIN_ROOT}/scripts/sw-run.py" capability_docs.pygenerate`
