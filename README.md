...
- **Workflow invariants** — `INVARIANTS.md` documents four enforcement-backed guarantees (required-capability nonskip, monotone re-detect, authorized reduction only, absolute floor); regenerate `CAPABILITIES.md` via `python3 "${CURSOR_PLUGIN_ROOT}/scripts/sw-run.py" capability_docs.py generate`
...
- **Workflow intelligence invariants** — four fail-closed assurance rules with enforcement paths in [`INVARIANTS.md`](INVARIANTS.md); triage monotonic merge in `scripts/triage_lib.py`; TraceRef/CoverageEdge predicates in `scripts/graph/traceability.py`
...
façades. Helpers resolve from the installed plugin via install-root `sw-run.py`:
...
resolve through the installed plugin via install-root `sw-run.py`
(`python3 "${CURSOR_PLUGIN_ROOT}/scripts/sw-run.py" <helper> [-- ARGS]`).
