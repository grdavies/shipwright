...
if [[ ! -f "$PRESETS" ]]; then
  for candidate in \
    "${CURSOR_PLUGIN_ROOT:-}/core/sw-reference/verify-presets.json" \
    "${CLAUDE_PLUGIN_ROOT:-}/core/sw-reference/verify-presets.json"; do
...
exec python3 - "$ROOT" "$PRESETS" "$PROPOSE" <<'PY'
import json
import re
import sys
