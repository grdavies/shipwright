"""Pytest port of run_deliver_fixtures.py (PRD 054 W3 behavioral)."""
...
def _load_harness(repo_root: Path):
    ...
    spec = importlib.util.spec_from_file_location("harness_deliver", path)
...
def test_deliver_behavior(repo_root: Path, sw_env: dict[str, str], monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_deliver_harness_present(repo_root: Path) -> None:
