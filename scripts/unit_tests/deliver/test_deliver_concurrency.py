"""Pytest port of run_deliver_concurrency_fixtures.py (PRD 054 W3 behavioral)."""
...
_HARNESS = "harness_deliver_concurrency.py"
...
def _load_harness(repo_root: Path):
    ...
    spec = importlib.util.spec_from_file_location("harness_deliver_concurrency", path)
...
def test_deliver_concurrency_behavior(repo_root: Path, sw_env: dict[str, str], monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_deliver_concurrency_harness_present(repo_root: Path) -> None:
