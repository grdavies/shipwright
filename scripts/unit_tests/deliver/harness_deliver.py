...
if grep -q '\-\-phase-mode' "$SW_SHIP" && grep -q 'SW_PHASE_MODE' "$SW_SHIP" && \
   grep -q 'merge-ready-green' "$SW_SHIP" && grep -q 'ship-phase-status' "$SW_SHIP"; then
...
if OUT=$(python3 "$SHIP_STATUS" --verdict blocked --cause "verification-gate:not-verified" --phase test-phase --out "$PDIR/blocked.json" 2>/dev/null) && \
   echo "$OUT" | python3 -c "
import json,sys
d=json.load(sys.stdin)
...
set +e
python3 "$SHIP_STATUS" --verdict blocked --phase x --out "$PDIR/x.json" 2>/dev/null
EC_BLK=$?
set -e
if [[ "$EC_BLK" -eq 2 ]]; then
...
if grep -q 'wave.py schedule' "$DELIVER_SKILL" && grep -q 'parallelCeiling' "$DELIVER_SKILL"; then
...
while IFS= read -r line || [[ -n "$line" ]]; do
  ...
  scenario="${line#*:}"
  if grep -qE "${scenario}" "$ROOT/scripts/unit_tests/deliver/harness_deliver.py"; then
...
if grep -q '/sw-deliver run' "$ROOT/core/documentation/commands.md" && \
   grep -q 'terminal merge gate' "$ROOT/core/documentation/commands.md"; then
