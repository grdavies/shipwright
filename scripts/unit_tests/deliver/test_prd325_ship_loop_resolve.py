"""PRD 325 R8–R9 — ship_loop resolution, dispatch env, and phase provision."""
...
from sw_scripts_resolve import CONSUMER_NO_PLUGIN_ERROR, ScriptsResolveError, validate_env_scripts_root
...
def test_untrusted_shipwright_scripts_fails_closed(tmp_path: Path) -> None:
    untrusted = tmp_path / "untrusted-scripts"
    untrusted.mkdir()
    (untrusted / "check-gate.py").write_text("# incomplete\n", encoding="utf-8")
    path, err = validate_env_scripts_root(str(untrusted))
    assert path is None
...
def test_build_ship_dispatch_child_env_scrubs_ambient_phase_keys() -> None:
    parent = {
        "SW_PHASE_MODE": "1",
        "SW_PHASE_SLUG": "stale",
        "SW_RUN_DIR": "/tmp/stale",
        "PATH": "/usr/bin",
        "HOME": "/tmp/home",
    }
    dispatch = {"SW_PHASE_MODE": "1", "SW_PHASE_SLUG": "fresh"}
    child = wdl.build_ship_dispatch_child_env(dispatch, parent=parent)
    assert child["SW_PHASE_SLUG"] == "fresh"
