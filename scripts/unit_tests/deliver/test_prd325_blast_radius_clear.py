...
def harness(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr("wave_failure.load_plan", lambda _root, _state=None: _plan())
    monkeypatch.setattr(
...
def test_shared_predicate_conservative_on_indeterminate_host() -> None:
    meta = {
        "slug": "dep",
        "status": "green-merged",
        "branch": "feat/dep",
    ...
    with patch("wave_phase_pr.phase_pr_host_merge_verdict", return_value={"verdict": "indeterminate"}):
        assert phase_green_merged(ROOT, meta) is False


def test_blast_radius_apply_clears_green_merged_dependent(harness) -> None:
    tmp_path, state, saved = harness
    ...
    assert blast["cleared"][0]["phaseSlug"] == "dep-merged"
    assert blast["cleared"][0]["reason"] == "green-merged"
    assert len(blast["applied"]) == 1
    assert blast["applied"][0]["phaseSlug"] == "dep-open"


def test_blast_radius_apply_all_green_is_noop(harness, monkeypatch: pytest.MonkeyPatch) -> None:
    tmp_path, state, saved = harness
    state["phases"]["3"]["status"] = "green-merged"
    state["phases"]["3"]["openPrNumber"] = 103

    def _all_merged(_root: Path, meta: dict[str, Any]) -> dict[str, Any]:
        return {"verdict": "ok", "merged": True}

    monkeypatch.setattr("wave_phase_pr.phase_pr_host_merge_verdict", _all_merged)
    with pytest.raises(SystemExit) as exc:
        cmd_blast_radius_apply(tmp_path, ["--phase-slug", "upstream"])
    assert exc.value.code == 0
    final = saved[-1]
    assert final["phases"]["2"]["status"] == "green-merged"
    assert final["phases"]["3"]["status"] == "green-merged"
    assert final["blastRadius"]["applied"] == []
    assert len(final["blastRadius"]["cleared"]) == 2


def test_blast_radius_dependents_reports_cleared(harness, capsys: pytest.CaptureFixture[str]) -> None:
    tmp_path, _state, _saved = harness
    with pytest.raises(SystemExit) as exc:
        cmd_blast_radius_dependents(tmp_path, ["--phase-slug", "upstream"])
    assert exc.value.code == 0
    payload = json.loads(capsys.readouterr().out)
    assert payload["blastRadius"]["cleared"][0]["reason"] == "green-merged"
    assert payload["dependentPhaseSlugs"] == ["dep-open"]
