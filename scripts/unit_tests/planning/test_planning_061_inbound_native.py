"""Pytest port — PRD 061 inbound comment sync + namespaced native ids (R18, R19)."""
...
_HARNESS = "harness_planning_061_inbound_native.py"
...
def _load(repo_root: Path):
    ...
    spec = importlib.util.spec_from_file_location("_h061_inbound", path)
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
    return mod


def test_planning_061_inbound_native(repo_root: Path, sw_env: dict, monkeypatch: pytest.MonkeyPatch) -> None:
