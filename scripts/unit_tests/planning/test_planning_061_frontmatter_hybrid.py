"""Pytest port — PRD 061 hybrid frontmatter + structural projection (R20–R22)."""
...
_HARNESS = "harness_planning_061_frontmatter_hybrid.py"
...
def _load(repo_root: Path):
    ...
    spec = importlib.util.spec_from_file_location("_h061_fm", path)
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
    return mod


def test_planning_061_frontmatter_hybrid(repo_root: Path, sw_env: dict, monkeypatch: pytest.MonkeyPatch) -> None:
