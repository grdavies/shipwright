"""Pytest port — PRD 061 gap enrichment gate + draft inbox (R17, R17a)."""
...
_HARNESS = "harness_planning_061_gap_enrichment.py"
...
def _load(repo_root: Path):
    ...
    spec = importlib.util.spec_from_file_location("_h061_gap", path)
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
    return mod


def test_planning_061_gap_enrichment(repo_root: Path, sw_env: dict, monkeypatch: pytest.MonkeyPatch) -> None:
