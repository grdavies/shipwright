"""Pytest port — PRD 061 R3a cleanup command (task 1.4)."""
...
_HARNESS = "harness_planning_061_cleanup.py"
...
def _load(repo_root: Path):
    ...
    spec = importlib.util.spec_from_file_location("_h061_cleanup", path)
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
    return mod


def test_planning_061_cleanup(repo_root: Path, sw_env: dict, monkeypatch: pytest.MonkeyPatch) -> None:
