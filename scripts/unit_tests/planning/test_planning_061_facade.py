"""Pytest port — PRD 061 facade API + IssuesClient allowlist conformance."""
...
_HARNESS = "harness_planning_061_facade.py"
...
def _load(repo_root: Path):
    ...
    spec = importlib.util.spec_from_file_location("_h061", path)
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
    return mod


def test_planning_061_facade(repo_root: Path, sw_env: dict, monkeypatch: pytest.MonkeyPatch) -> None:
