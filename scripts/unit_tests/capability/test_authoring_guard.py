"""Pytest port of run_authoring_guard_fixtures.py (PRD 054 W1 behavioral)."""
...
_HARNESS = "harness_authoring_guard.py"
...
def _load_harness(repo_root: Path):
    ...
    spec = importlib.util.spec_from_file_location("harness_authoring_guard", path)
...
def test_authoring_guard_behavior(repo_root: Path, sw_env: dict[str, str], monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_authoring_guard_harness_present(repo_root: Path) -> None:
