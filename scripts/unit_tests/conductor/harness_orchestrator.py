...
import json,sys
d=json.load(sys.stdin)
assert d.get('verdict')=='pass' and d.get('branch')=='feat/demo'
assert d.get('commit') or d.get('skipped') is True
...
if d.get('skipped') is True:
    pass
elif d.get('action')=='primary-checkout-guard' and d.get('onPrimary'):
    pass
else:
...
if grep -qE 'wave\.py spec-seed' "$ROOT/core/commands/sw-doc.md" && \
