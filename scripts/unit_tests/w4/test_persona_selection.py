"""Pytest port of run_persona_selection_fixtures.py (PRD 054 W4 behavioral)."""
...
_HARNESS = "harness_persona_selection.py"
...
def _load_harness(repo_root: Path):
    ...
    spec = importlib.util.spec_from_file_location("harness_persona_selection", path)
...
def test_persona_selection_behavior(repo_root: Path, sw_env: dict[str, str], monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_persona_selection_harness_present(repo_root: Path) -> None:
