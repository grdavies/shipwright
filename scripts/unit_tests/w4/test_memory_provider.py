"""Pytest port of run_memory_provider_fixtures.py (PRD 054 W4 behavioral)."""
...
_HARNESS = "harness_memory_provider.py"
...
def _load_harness(repo_root: Path):
    ...
    spec = importlib.util.spec_from_file_location("harness_memory_provider", path)
...
def test_memory_provider_behavior(repo_root: Path, sw_env: dict[str, str], monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_memory_provider_harness_present(repo_root: Path) -> None:
