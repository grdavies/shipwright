"""Pytest port of run_impl_fixtures.py (PRD 054 W4 behavioral)."""
...
def _load_harness(repo_root: Path):
    ...
    spec = importlib.util.spec_from_file_location("harness_impl", path)
...
def test_impl_behavior(repo_root: Path, sw_env: dict[str, str], monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_impl_harness_present(repo_root: Path) -> None:
