"""Pytest port of run_improvement_fixtures.py (PRD 054 W4 behavioral)."""
...
_HARNESS = "harness_improvement.py"
...
def _load_harness(repo_root: Path):
    ...
    spec = importlib.util.spec_from_file_location("harness_improvement", path)
...
def test_improvement_behavior(repo_root: Path, sw_env: dict[str, str], monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_improvement_harness_present(repo_root: Path) -> None:
