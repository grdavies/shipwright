"""Pytest port of run_doc_currency_033_fixtures.py (PRD 054 W4 behavioral)."""
...
_HARNESS = "harness_doc_currency_033.py"
...
def _load_harness(repo_root: Path):
    ...
    spec = importlib.util.spec_from_file_location("harness_doc_currency_033", path)
...
def test_doc_currency_033_behavior(repo_root: Path, sw_env: dict[str, str], monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_doc_currency_033_harness_present(repo_root: Path) -> None:
