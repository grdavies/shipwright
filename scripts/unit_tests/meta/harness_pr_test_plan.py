...
GATE_LIB="$ROOT/scripts/check_gate_lib.py"
...
python3 - "$MANIFEST" "$GATE_LIB" <<'PY' || { bad "pr-test-plan-blocking-classification: manifest/gate script"; }
...
assert "requiredFailingChecks" in gate and "advisoryFailingChecks" in gate
...
if grep -q 'check-gate.py' "$STABILIZE_SKILL" && \
