...
if grep -q 'dispatch-preflight binding overrides session-start' "$SESSION_CTX" && \
   grep -q 'resolve-intensity.sh' "$TIERING"; then
...
if grep -q 'git-push.sh' "$CONDUCTOR_SKILL" && \
   grep -q 'git-push.sh' "$CONDUCTOR_RULE" && \
...
for cmd in sw-ship sw-debug sw-doc sw-feedback; do
  f="$ROOT/core/commands/${cmd}.md"
  if ! grep -q 'dispatch-check.sh' "$f" || ! grep -q 'resolve-intensity.sh' "$f"; then
    OD_FAIL=1
    break
  fi
done
...
if grep -q 'delegate-by-default' "$DISPATCH_RULE" && \
   grep -q 'dispatch-check.sh' "$DISPATCH_RULE" && \
   grep -q 'resolve-intensity.sh' "$TIERING" && \
