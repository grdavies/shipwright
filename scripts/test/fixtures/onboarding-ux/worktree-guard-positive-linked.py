...
_SOURCE = r"""
#!/usr/bin/env bash
# Positive: linked worktree with phase branch allows implementation entry (R27).
set -euo pipefail
...
git -C "$TMP/repo" worktree add -q "$TMP/wt" -b feat/phase-test

set +e
OUT=$(cd "$TMP/wt" && bash "$GUARD" 2>&1)
EC=$?
set -e

if [[ "$EC" -eq 0 ]]; then
  echo "OK  worktree-guard positive: linked worktree + feat branch"
  exit 0
fi

echo "FAIL worktree-guard positive linked expected exit=0 (ec=$EC)"
echo "$OUT"
exit 1
