...
| `/sw-ship` execute fan-out | `execute_plan.py` / `execute_ship.py` at phase entry | Execute step plan | Linear authored order + contention edges | PRD 053 default-on | `execute-plan-dag-049-phase-2`, `execute-ship-chain-gated`, `execute-plan-linear-fallback` |
