...
**Model tier:** build — resolve via `python3 scripts/sw_bootstrap.py resolve-model-tier.py -- --skill debug`. When using the Task tool for subagent dispatch, resolve concrete model IDs from `models.tiers` in config (never semantic tier names in subagent `model:` frontmatter).
...
Under `proposed`, Sentry bodies MUST be redacted via `python3 scripts/sw_bootstrap.py memory-redact.py` before any persist or
...
2. **Redact** all text: `python3 scripts/sw_bootstrap.py memory-redact.py`.
