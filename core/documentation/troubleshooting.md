...
## Issue-store projection timeout and rate limits (PRD 344)
...
Under load — the failure mode observed after PRD 337 merged — that search can stall or return
HTTP 429. Shipwright bounds the work instead of hanging indefinitely.
...
| Key | Role |
| --- | --- |
| `searchTimeoutSeconds` | Per-request timeout for GitHub **search** calls (R1) |
| `searchMaxCumulativeWaitMs` | Cap on cumulative 429 backoff for search (R2) |
...
primary stays responsive (R3). Resume commands preserve that scoping.

### Validation fixture

Hermetic reproduction of the PRD 337 post-merge rate-limit scenario lives in
`scripts/unit_tests/planning/test_projection_performance.py`
(`prd337_post_merge_rate_limit_scenario`). It does not call the live API.
