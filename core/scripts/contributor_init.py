...
def validate_docs(*, root: Path) -> dict[str, Any]:
    ...
    if readme.is_file():
        ...
        # Accept uv tool, pipx, or pip install patterns for adopters (PRD 345 R9)
        has_adopter_install = (
            "uv tool install" in text
            or "pipx install" in text
            or "pip install shipwright-workflow" in text
        )
        if not has_adopter_install:
