"""Pytest port of run_inflight_signal_fixtures.py (PRD 054 W2 behavioral)."""
...
_HARNESS = "harness_inflight_signal.py"
...
def _load_harness(repo_root: Path):
    ...
    spec = importlib.util.spec_from_file_location("harness_inflight_signal", path)
...
@pytest.mark.git
def test_inflight_signal_tmp_git_repo_ready(tmp_git_repo: Path) -> None:
    """R15 — shared tmp_git_repo fixture is usable for W2 git scenarios."""
    assert (tmp_git_repo / ".git").is_dir()


@pytest.mark.git
def test_inflight_signal_behavior(repo_root: Path, sw_env: dict[str, str], monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_inflight_signal_harness_present(repo_root: Path) -> None:
