"""Pytest port of run_deliver_loop_fixtures.py (PRD 054 W3 behavioral)."""
...
_HARNESS = "harness_deliver_loop.py"
...
def _load_harness(repo_root: Path):
    ...
    spec = importlib.util.spec_from_file_location("harness_deliver_loop", path)
...
def test_deliver_loop_behavior(repo_root: Path, sw_env: dict[str, str], monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_deliver_loop_harness_present(repo_root: Path) -> None:
