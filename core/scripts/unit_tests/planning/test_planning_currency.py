"""Pytest port of run_planning_currency_fixtures.py (PRD 054 W4 behavioral)."""
...
_HARNESS = "harness_planning_currency.py"
...
def _load_harness(repo_root: Path):
    ...
    spec = importlib.util.spec_from_file_location("harness_planning_currency", path)
...
def test_planning_currency_behavior(repo_root: Path, sw_env: dict[str, str], monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_planning_currency_harness_present(repo_root: Path) -> None:
