"""Pytest port of run_portability_closure_fixtures.py (PRD 054 W4 behavioral)."""
...
_HARNESS = "harness_portability_closure.py"
...
def _load_harness(repo_root: Path):
    ...
    spec = importlib.util.spec_from_file_location("harness_portability_closure", path)
...
def test_portability_closure_behavior(repo_root: Path, sw_env: dict[str, str], monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_portability_closure_harness_present(repo_root: Path) -> None:
