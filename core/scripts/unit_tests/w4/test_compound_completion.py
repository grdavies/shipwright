"""Pytest port of run_compound_completion_fixtures.py (PRD 054 W4 behavioral)."""
...
_HARNESS = "harness_compound_completion.py"
...
def _load_harness(repo_root: Path):
    ...
    spec = importlib.util.spec_from_file_location("harness_compound_completion", path)
...
def test_compound_completion_behavior(repo_root: Path, sw_env: dict[str, str], monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_compound_completion_harness_present(repo_root: Path) -> None:
