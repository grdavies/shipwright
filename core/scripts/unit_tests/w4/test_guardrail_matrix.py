"""Pytest port of run_guardrail_matrix_fixtures.py (PRD 054 W4 behavioral)."""
...
_HARNESS = "harness_guardrail_matrix.py"
...
def _load_harness(repo_root: Path):
    ...
    spec = importlib.util.spec_from_file_location("harness_guardrail_matrix", path)
...
def test_guardrail_matrix_behavior(repo_root: Path, sw_env: dict[str, str], monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_guardrail_matrix_harness_present(repo_root: Path) -> None:
