"""Pytest port of run_code_review_fixtures.py (PRD 054 W4 behavioral)."""
...
_HARNESS = "harness_code_review.py"
...
def _load_harness(repo_root: Path):
    ...
    spec = importlib.util.spec_from_file_location("harness_code_review", path)
...
def test_code_review_behavior(repo_root: Path, sw_env: dict[str, str], monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_code_review_harness_present(repo_root: Path) -> None:
