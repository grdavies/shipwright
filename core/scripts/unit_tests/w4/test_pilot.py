"""Pytest port of run_pilot_fixtures.py (PRD 054 W4 behavioral)."""
...
def _load_harness(repo_root: Path):
    ...
    spec = importlib.util.spec_from_file_location("harness_pilot", path)
...
def test_pilot_behavior(repo_root: Path, sw_env: dict[str, str], monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_pilot_harness_present(repo_root: Path) -> None:
