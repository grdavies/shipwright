...
/sw-deliver run --issue <issue-number> --issue-source planning-store|host-repo
```
...
**Issue-source disambiguation (PRD 339 R38):** under `separate-project` issue-store, `run --issue N`
probes both the planning-store project and the host code repo. When both repos expose the same issue
number, entry **fails closed** with `halt: issue-source-collision` until the operator passes an explicit
selector:

| Selector | Resolves from |
| --- | --- |
| `--issue-source planning-store` | `planning.store.storeLocation` project (semantic planning artifacts) |
| `--issue-source host-repo` | Host provider remote (`host.remote` / `origin`) |

Run entry prints `issue-source=<origin> sw-unit-id=<unit-id>` on stderr and embeds
`issueResolution` in the JSON envelope. Warn-only collision handling is prohibited — ambiguous
same-number issues never auto-resolve.
...
<!-- currency: refreshed 2026-09-14T20:02:00Z — host-switch terminal prepare vs wave_deliver_loop + wave_terminal -->
