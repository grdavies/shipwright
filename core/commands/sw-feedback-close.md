...
3. Run eligibility gate:
   ...
   ```bash
   python3 scripts/feedback-closure-gate.sh \
...
5. On `closable`, close backlog entry:
   ...
   ```bash
   python3 scripts/feedback-backlog.sh close --signal-id <id> --backlog docs/prds/GAP-BACKLOG.md
   ```
...
**Model tier:** mid — resolve via `python3 scripts/resolve-model-tier.sh --command sw-feedback-close`.
