...
5. **E2E/smoke adapter** (when `verifyE2e.enabled`):
   ...
   ```bash
   E2E_JSON=$(python3 scripts/verify-e2e.sh)
   E2E_EC=$?
...
6. After all commands complete, resolve the run dir and write the aggregate status file:
   ...
   Optional baseline capture (off by default): `python3 scripts/verify-baseline.sh capture --from "$STATUS_FILE" --to <caller-owned-baseline>`.
...
**Model tier:** cheap — resolve via `python3 scripts/resolve-model-tier.sh --command sw-verify`.
