...
1. **Worktree guard** — run `python3 scripts/sw-assert-worktree.sh` before any implementation write. Exit `1` → halt
...
3. Load the task file from `tasksDir` for this phase; resolve requirements via **spec union**:
   ...
   ```bash
   python3 scripts/spec-union.sh <frozen-prd-path>
   ```

   Load `skills/spec-union/SKILL.md`. Parse `## Traceability` for R-ID → test scenario per task ref.
   Load open `docs/prds/GAP-BACKLOG.md` items (`python3 scripts/feedback-backlog.sh list --open-only`) linked to
...
7. **Per task ref** (sub-task granularity, e.g. `1.1`, `1.2`):
   1. `python3 scripts/plan-self-review.sh --tasks <task-file> --task-ref <ref>` — halt on `fail`.
   ...
   5. `python3 scripts/tdd-gate.sh --status /tmp/sw-tdd.status.json` — halt on `fail` (`20`).
...
**Model tier:** build — resolve via `python3 scripts/resolve-model-tier.sh --command sw-execute`.
