...
```bash
python3 scripts/memory-sot-audit.sh audit-conflicts
# optional one-time migration plan when switching memory.sourceOfTruth:
python3 scripts/memory-sot-audit.sh legacy-reconcile-plan --target memory
```
...
**Model tier:** mid — resolve via `python3 scripts/resolve-model-tier.sh --command sw-memory-audit`.
...
- Resolve git↔provider authority per `python3 scripts/memory-sot.sh resolve --class decision --json` — flag
