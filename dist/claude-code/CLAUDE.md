...
- **Mechanical** — reconciler-generated artifacts only (INDEX `derived` region, SUPERSEDED manifest, gap index):
  ...
  Under issue-store `separate-project` (PRD 057 R1–R3) the mechanical allowlist **projects to the authoritative
  store** instead — gap capture write-through (R1), spec-seed skips the local INDEX write (R2), and
  `reconcile_core` skips tracked `INDEX.md`/`INDEX-archive.md`/`SUPERSEDED.md`/legacy-projection writes and
  projects the derived map to the store or a gitignored cache (R3). `same-repo` retains local mechanical writes
  unchanged.
...
- Core skill `name` in `SKILL.md` frontmatter MUST equal the unprefixed kebab-case directory name (`core/skills/<dir>/` → `name: <dir>`).
- The `sw-` prefix is reserved for commands and agents, not skill identity.
