"""Pytest port of run_hook_fixtures.py (PRD 054 W3 behavioral)."""
...
def _load_harness(repo_root: Path):
    ...
    spec = importlib.util.spec_from_file_location("harness_hook", path)
...
def test_hook_behavior(repo_root: Path, sw_env: dict[str, str], monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_hook_harness_present(repo_root: Path) -> None:
