"""Pytest port of run_portability_setup_fixtures.py (PRD 054 W4 behavioral)."""
...
_HARNESS = "harness_portability_setup.py"
...
def _load_harness(repo_root: Path):
    ...
    spec = importlib.util.spec_from_file_location("harness_portability_setup", path)
...
def test_portability_setup_behavior(repo_root: Path, sw_env: dict[str, str], monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_portability_setup_harness_present(repo_root: Path) -> None:
