"""Pytest port of run_capability_fixtures.py (PRD 054 W4 behavioral)."""
...
_HARNESS = "harness_capability.py"
...
def _load_harness(repo_root: Path):
    ...
    spec = importlib.util.spec_from_file_location("harness_capability", path)
...
def test_capability_behavior(repo_root: Path, sw_env: dict[str, str], monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_capability_harness_present(repo_root: Path) -> None:
