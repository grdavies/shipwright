"""Pytest port of run_behavioral_anomaly_fixtures.py (PRD 054 W4 behavioral)."""
...
_HARNESS = "harness_behavioral_anomaly.py"
...
def _load_harness(repo_root: Path):
    ...
    spec = importlib.util.spec_from_file_location("harness_behavioral_anomaly", path)
...
def test_behavioral_anomaly_behavior(repo_root: Path, sw_env: dict[str, str], monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_behavioral_anomaly_harness_present(repo_root: Path) -> None:
