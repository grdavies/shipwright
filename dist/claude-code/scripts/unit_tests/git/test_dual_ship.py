"""Pytest port of run_dual_ship_fixtures.py (PRD 054 W2 behavioral)."""
...
def _load_harness(repo_root: Path):
    ...
    spec = importlib.util.spec_from_file_location("harness_dual_ship", path)
...
@pytest.mark.git
def test_dual_ship_tmp_git_repo_ready(tmp_git_repo: Path) -> None:
    """R15 — shared tmp_git_repo fixture is usable for W2 git scenarios."""
    assert (tmp_git_repo / ".git").is_dir()


@pytest.mark.git
def test_dual_ship_behavior(repo_root: Path, sw_env: dict[str, str], monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_dual_ship_harness_present(repo_root: Path) -> None:
