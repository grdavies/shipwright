"""Pytest port of run_plan_persist_fixtures.py (PRD 054 W4 behavioral)."""
...
_HARNESS = "harness_plan_persist.py"
...
def _load_harness(repo_root: Path):
    ...
    spec = importlib.util.spec_from_file_location("harness_plan_persist", path)
...
def test_plan_persist_behavior(repo_root: Path, sw_env: dict[str, str], monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_plan_persist_harness_present(repo_root: Path) -> None:
