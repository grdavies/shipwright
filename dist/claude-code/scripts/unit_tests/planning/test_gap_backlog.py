"""Pytest port of run_gap_backlog_fixtures.py (PRD 054 W1 behavioral)."""
...
_HARNESS = "harness_gap_backlog.py"
...
def _load_harness(repo_root: Path):
    ...
    spec = importlib.util.spec_from_file_location("harness_gap_backlog", path)
...
def test_gap_backlog_behavior(repo_root: Path, sw_env: dict[str, str], monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_gap_backlog_harness_present(repo_root: Path) -> None:
