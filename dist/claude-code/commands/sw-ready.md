...
1. `bash scripts/host.sh pr-view` for number, url, draft/base/head; combine with `scripts/check-gate.py` for checks.
...
5. **Authoritative gate** — do not hand-roll:
   ...
   ```bash
   GATE="${CLAUDE_PLUGIN_ROOT:-$PWD}/scripts/check-gate.py"
