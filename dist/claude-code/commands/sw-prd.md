...
0. **Authoring-guard preflight (PRD 032 R5/R14)** — before the first substantive mutation on a planning unit, run `python3 scripts/authoring-guard.sh preflight --path <unit-artifact> --command sw-prd`; on a genuinely in-flight unit, pass `--handoff <reason>` instead of mutating (R6).
...
3. Resolve `--type` (default `prd`) and section contract from the skill.
   ...
   - **Pull-in proposal (R1/R17):** before drafting, run
     `python3 scripts/planning-related.sh scan --mode creation --path <unit-artifact>`; surface the
     **confirm-list** (never auto-absorb). Flag stale/already-resolved candidates; human confirms via
     `python3 scripts/planning-related.sh confirm --path <unit-artifact> --accept <id>[,...]`. Frozen targets
...
4. **PRD (`--type prd`, default):**
   ...
   - **Structural canonicalization (R14):** before persisting the draft, pipe body output through
     `python3 scripts/doc-format-normalize.sh --write --inplace <path>` so non-canonical structural shape cannot
...
**Model tier:** deep — resolve via `python3 scripts/resolve-model-tier.sh --command sw-prd`.
