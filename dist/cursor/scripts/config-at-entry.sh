...
# Usage: config-at-entry.py [--quiet]
set -euo pipefail
...
[[ -x "$ROOT/scripts/sw-configure.py" ]] || exit 0
OUT="$(bash "$ROOT/scripts/sw-configure.py" drift-check --config "${CONFIG:-/nonexistent}" 2>/dev/null || echo '{}')"
