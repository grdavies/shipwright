...
Output: JSON { "tier", "modelId", "source" }
  ...
  --agent ID        — reviewer/persona/native-panel agent id (e.g. sw-coherence-reviewer)
...
tier="" command="" skill="" agent="" delegate="" config=""
while [[ $# -gt 0 ]]; do
  case "$1" in
    --tier) tier="$2"; shift 2 ;;
...
if [[ -z "$tier" && -z "$command" && -z "$skill" && -z "$agent" ]]; then
  usage >&2
  exit 2
fi
...
exec python3 - "$tier" "$command" "$skill" "$agent" "$delegate" "$config" "$DEFAULTS" <<'PY'
import json
import sys
