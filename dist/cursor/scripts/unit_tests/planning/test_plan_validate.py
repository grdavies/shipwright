"""Pytest port of run_plan_validate_fixtures.py (PRD 054 W1 behavioral)."""
...
_HARNESS = "harness_plan_validate.py"
...
def _load_harness(repo_root: Path):
    ...
    spec = importlib.util.spec_from_file_location("harness_plan_validate", path)
...
def test_plan_validate_behavior(repo_root: Path, sw_env: dict[str, str], monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_plan_validate_harness_present(repo_root: Path) -> None:
