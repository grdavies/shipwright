"""Pytest port of run_planning_init_fixtures.py (PRD 054 W4 behavioral)."""
...
_HARNESS = "harness_planning_init.py"
...
def _load_harness(repo_root: Path):
    ...
    spec = importlib.util.spec_from_file_location("harness_planning_init", path)
...
def test_planning_init_behavior(repo_root: Path, sw_env: dict[str, str], monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_planning_init_harness_present(repo_root: Path) -> None:
