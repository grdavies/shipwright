"""Pytest port of run_relocation_fixtures.py (PRD 054 W4 behavioral)."""
...
_HARNESS = "harness_relocation.py"
...
def _load_harness(repo_root: Path):
    ...
    spec = importlib.util.spec_from_file_location("harness_relocation", path)
...
@pytest.mark.git
def test_relocation_tmp_git_repo_ready(tmp_git_repo: Path) -> None:
    assert (tmp_git_repo / ".git").is_dir()


@pytest.mark.git
def test_relocation_behavior(repo_root: Path, sw_env: dict[str, str], monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_relocation_harness_present(repo_root: Path) -> None:
