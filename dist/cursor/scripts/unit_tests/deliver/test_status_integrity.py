"""Pytest port of run_status_integrity_fixtures.py (PRD 054 W1 behavioral)."""
...
_HARNESS = "harness_status_integrity.py"
...
def _load_harness(repo_root: Path):
    ...
    spec = importlib.util.spec_from_file_location("harness_status_integrity", path)
...
def test_status_integrity_behavior(repo_root: Path, sw_env: dict[str, str], monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_status_integrity_harness_present(repo_root: Path) -> None:
