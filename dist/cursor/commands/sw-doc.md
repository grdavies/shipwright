...
```bash
python3 scripts/docs_worktree.sh provision --topic <topic>
```
...
| --- | --- | --- |
| Mechanical | INDEX `derived` region, SUPERSEDED manifest, generated gap index only | `python3 scripts/docs-edit-route.sh route --path …` → `docs-merge.sh` |
| Substantive | Any `docs/planning/<unit-id>/` path (body or frontmatter) | `python3 scripts/docs-edit-route.sh route-substantive --topic <topic>` |
...
**Model tier:** inherit — resolve delegated atomics via `python3 scripts/resolve-model-tier.sh --command <child-slug>`; do not dispatch on bare `--command sw-doc`.
...
2. `python3 scripts/dispatch-check.sh --agent <agent-id> --command sw-doc --skill <active-skill> --parent-model <parent-concrete-id> [--dispatch-id <id>]`
3. Pass explicit `model: <resolved-concrete-id>` on Task input (never `inherit`).

Resolve model: `python3 scripts/resolve-model-tier.sh --command <child-slug>` (or `--agent` for doc-review personas).
...
Before building a Task prompt, route non-config context through `python3 scripts/memory-redact.sh` and embed
