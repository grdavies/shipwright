...
1. `python3 scripts/reconcile-status.sh derive` — show per-PRD status + task/PR linkage.
...
5. **Verify-unconfigured (R28)** — run `python3 scripts/verify-unconfigured.sh`; include signal + CTA (`run /sw-init`) when unconfigured.
6. **Config drift (R32)** — run `python3 scripts/sw-configure.sh drift-check`; surface stale notice when applicable.
...
8. **Deliver runs (R10)** — `python3 scripts/reconcile-status.sh deliver-runs` lists every live scoped deliver
...
**Model tier:** cheap — resolve via `python3 scripts/resolve-model-tier.sh --command sw-status`.
