...
**Model tier:** mid — resolve via `python3 scripts/resolve-model-tier.sh --skill gap-check`. When using the Task tool for subagent dispatch, resolve concrete model IDs from `models.tiers` in config (never semantic tier names in subagent `model:` frontmatter).

## Inputs

- **Plan:** task checklist for `phaseSlug` in `tasksDir` + spec union (`scripts/spec-union.sh <prd>`).
- **Backlog:** open rows from `python3 scripts/feedback-backlog.sh list --open-only` (`skills/feedback-closure/SKILL.md`) — map against diff when PR-linked.
- **Native panel advisory (R75):** when present, read `$runDir/sw-local-review-run-report.json` (resolved via
  `python3 scripts/sw-tmp.sh resolve` or `shipwright-state` `runDir`) and consume `scope_fidelity_advisory` **advisory
...
```bash
PARENT=$(python3 scripts/shipwright-state.sh read | Python json -r .parentBranch)
