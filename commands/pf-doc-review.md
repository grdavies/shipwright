...
| Input | Panel |
|-------|-------|
| PRD draft (`prds/...`) | Signal-driven core + gated specialists (see skill) |
| Decision-record draft (`decisions/<n>-<slug>.md`) | **Full** — all seven personas (cross-cutting blast radius) |
| Amendment under `prds/.../amendments/` | Coherence + scope-guardian (generic floor) |
| Amendment under `decisions/...amendments/` | Raised floor: coherence + scope-guardian + adversarial + feasibility (+ security when auth/data/migrations) |
...
2. **Model tier (R9):** before dispatching persona subagents, verify the parent agent's model tier is ≥
   `models.roles.builder` from config (see `docs/models-tiering.md`). Reviewer agents use `model: inherit` —
...
4. Detect doc type from path:
   - `decisions/<n>-<slug>.md` (not under `.amendments/`) → decision-record **draft** → Full panel (all seven).
   - `decisions/<n>-<slug>.amendments/A<k>-*.md` → decision **amendment** → raised floor per skill.
   - `.../amendments/A<k>-*.md` under `prds/` → PRD amendment → coherence + scope-guardian only (U7).
...
7. **Decision-record drafts:** dispatch all seven `agents/pf-*-reviewer.md` personas (equivalent to `--all`).
...
13. Report result; next step `/pf-freeze` when clear.
...
- Decision amendments: raised floor only for `decisions/` parents — PRD amendment floor unchanged.
- Quick: no panel.
