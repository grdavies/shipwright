...
- Output: updated backlog checkbox + `/tmp/pf-feedback-close.status.json`.
- Does **not** merge, push, or re-run `/pf-feedback` routing.

## Procedure

1. Load `skills/feedback-closure/SKILL.md`.
2. **Human confirm** — surface signal summary; proceed only on explicit user OK (same bar as `/pf-feedback` dispatch).
...
7. Emit `/tmp/pf-feedback-close.status.json` with `{signalId, verdict: closed, date}`.
