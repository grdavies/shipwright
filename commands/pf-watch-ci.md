...
alwaysApply: false
trigger: "/pf-watch-ci" or "watch CI for the current PR"
---
...
```bash
GATE="${CURSOR_PLUGIN_ROOT:-$HOME/.cursor/plugins/local/phase-flow-v2}/scripts/check-gate.sh"
...
6. Act on the verdict:
   - **`yellow`** (checks pending or CodeRabbit in-flight) — arm a wake and block on CI settling rather
     ...
     CodeRabbit barrier and recompute. (Standalone `/pf-watch-ci` runs one wait-and-rewatch; under `/ship` the
     orchestrator owns the bounded loop and re-enters here.)

```bash
PR=$(gh pr view --json number --jq .number)
gh pr checks "$PR" --watch --fail-fast >/tmp/pf-watch-ci-watch.log 2>&1 || true
...
```
   ...
   - **`red`/`blocked`** — pull failure logs for the `/pf-stabilize` handoff:

```bash
RUN_IDS=$(gh run list --commit "$HEAD_SHA" --event pull_request --json databaseId --jq '.[].databaseId // empty')
: > /tmp/pf-watch-ci-failed.log
for RUN_ID in $RUN_IDS; do
  [ -n "$RUN_ID" ] || continue
  printf '=== failed logs for run %s ===\n' "$RUN_ID" >> /tmp/pf-watch-ci-failed.log
  gh run view "$RUN_ID" --log-failed >> /tmp/pf-watch-ci-failed.log || true
done
```
...
- `green` — all checks pass, none pending, CodeRabbit settled for the current head (`coderabbitLanded == true` — reviewed, **skipped**, or absent), zero unresolved actionable threads → `/pf-phase-ready`
- `red` — any check failed → `/pf-stabilize`
...
- `blocked` — unconfigured/neutral-blocking checks, or unresolved actionable threads → `/pf-stabilize`
...
- Store memories only for durable CI/review failure patterns (usually during `/pf-stabilize`), never for
