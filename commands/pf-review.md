...
6. Invoke adapter per `providers/code-review/ce-code-review.md`:
   ...
   Capture raw JSON to `/tmp/pf-local-review-raw.json`.
7. Normalize:

   ```bash
   scripts/code-review-normalize.sh --input /tmp/pf-local-review-raw.json --repo-root "$PWD" \
     > /tmp/pf-local-review-normalized.json
   ```
...
8. **Apply** (pf-owned, untrusted-output boundary):
...
9. **Bounded re-verify:** if any fix applied, run `/pf-verify` once. Circuit-breaker: three identical
   failures without diff change → escalate per `rules/pf-subagent-dispatch.mdc`.
10. **Severity gate:**

    ```bash
    scripts/code-review-gate.sh \
      --input /tmp/pf-local-review-normalized.json \
      --gate-config /tmp/pf-local-review-gate.json
    ```

    Write gate config from `review.local.gate`. Surface-only default (`haltOn: []`) logs P0–P3 and
    continues. Halting mode (`haltOn: ["P0","P1"]`) records halt signal for `/pf-ship` (validated
    P0/P1 only). Persist gate result to `/tmp/pf-local-review-gate-result.json`.
11. **Persist edges (redaction):**
    - Scrub `ce-code-review` run dir after parsing:

      ```bash
      RUN_DIR="$(jq -r '.artifact_path // empty' /tmp/pf-local-review-raw.json)"
