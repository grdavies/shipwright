...
Single-pass task list from the frozen PRD 050 spec union (R1–R19; decisions D6–D8). Phases mirror the PRD
Rollout Plan (Thread A → B/C parallel-eligible → D → CI/gap verification). No implementation starts until the
`doc.afterTasks` boundary.
...
- [ ] 5.1 Register all nine fixtures in pr-test-plan manifest
  - **File:** `core/sw-reference/pr-test-plan.manifest.json`, `.github/workflows/pr-test-plan-ci.yml`
  - **Expected:** all nine named scenarios from PRD Testing Strategy registered `required`; workflow regenerated if needed
  - **R-IDs:** R5, R6, R7, R9, R11, R13, R16, R17, R18
