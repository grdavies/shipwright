...
- [ ] 1.1 Add a branch-name guard single-sourcing allowed types from `release-please-config.json` (R22, R25)
...
- [ ] 1.2 Remove the `pf/<name>` default in `worktree.sh` and call the guard at provision (R23, R27)
...
- [ ] 1.3 Conforming multi-feature derivation with single-sourced types (R24)
...
- [ ] 1.4 Migrate existing `pf/` matchers and fixtures to the conforming scheme (R26)
...
- [ ] 2.1 Atomic, corruption-detecting state read/write (R43)
...
- [ ] 3.1 `deliver-loop` verb with durable cursor + idempotent resume (R1, R2, R3)
...
- [ ] 3.2 Remove manual handoff; wire `doc.afterTasks` directly to the driver (R4, R5)
...
- [ ] 3.3 Advance solely from durable per-phase status (R7)
...
- [ ] 3.4 Bounded remediation, blast-radius siblings, clean terminal + consolidated blocker (R8, R9, R10, R12)
...
- [ ] 3.5 Configurable remediation budget, default 2 (R11)
...
- [ ] 3.6 Heartbeat + per-phase timeout watchdog (R46)
...
- [ ] 4.1 Orchestrator owns a non-detached `<type>/<slug>`; primary asserted off; dirty-primary fails closed (R55, R40)
...
- [ ] 4.2 Single idempotent spec-seed owner for both entry paths (R6, R57)
...
- [ ] 5.1 `status collect` resolves the phase-worktree status path directly (R38)
...
- [ ] 5.2 No-PR local-merge path; branch on PR presence; honor gate/barrier (R39, R54)
...
- [ ] 5.3 Bind phase status to head SHA (R47)
...
- [ ] 6.1 Persist `/sw-ship` step-level state; resume mid-chain (R58)
...
- [ ] 7.1 Checkbox-only progress writer + non-checkbox-edit guard (R13, R14)
...
- [ ] 7.2 Frozen-guard checkbox carve-out (R48)
...
- [ ] 7.3 Durable per-task completion ledger (R49)
...
- [ ] 7.4 Currency gate hard-block + in-loop commit (R15, R16)
...
- [ ] 8.1 Pre-merge compound-ship mode; commit file outputs; memory not committed (R17, R18, R19)
...
- [ ] 8.2 Rule-class memory promotion stays human-gated (R21)
  - **File:** `core/commands/sw-compound-ship.md`
  - **Expected:** no auto-promotion of rule-class memory inside the loop
- [ ] 8.3 `completed-pending-merge` sub-state; completion gated on merge detection; suggest cleanup (R20, R31, R53)
...
- [ ] 9.1 Secret scan at every workflow push chokepoint (R41, R50)
...
- [ ] 9.2 Single-sourced patterns + allowlist + fail-closed (R51)
...
- [ ] 9.3 Mechanical range-scoped redaction guard + rule (R42, R52)
...
- [ ] 10.1 `/sw-cleanup` command + script: dry-run + confirm + report (R28, R29, R33)
...
- [ ] 10.2 Protections + no `rm -rf` (R30, R32)
...
- [ ] 10.3 Squash-aware merge detection + shared-state guards (R56)
  - **File:** `scripts/cleanup.sh`
...
- [ ] 10.4 Register in plugin manifest under the `sw-` contract (R34)
...
- [ ] 11.1 Fixture suite for all new behaviors (R36)
...
- [ ] 11.2 Documentation updates (R37)
...
- [ ] 11.3 Emitter propagation + freshness gate (R35)
