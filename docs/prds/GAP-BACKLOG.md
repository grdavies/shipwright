...
| GAP-009 | resolved | — | Model tier routing not bound at Task dispatch (R9 procedural only): |
...
| GAP-012 | resolved | — | `/sw-deliver` v1 deferrals still open: |
...
| GAP-016 | resolved | — | Frozen PRDs are not auto-committed to `main`, creating a data-loss windo |
...
| GAP-021 | resolved | — | Retrospective/compounding not auto-run before terminal PR in `/sw-delive |
| GAP-022 | resolved | — | Terminal PR + CI watch + stabilize should run autonomously; merge to `ma |
...
| GAP-026 | resolved | — | during PRD 012 phase-mode deliver, the agent halted with `Resume: bash |
| GAP-027 | resolved | — | Conductor paused after post-remediation green-merged phase ("status-paus |
| GAP-028 | resolved | — | `compoundShip` / `record-premerge` writes orchestrator worktree state bu |
| GAP-029 | resolved | — | Conductor paused mid-phase during `dispatch-ship` (incomplete ship work |
| GAP-030 | resolved | — | Phase worktrees not torn down immediately after `green-merged` — linger |
...
| GAP-039 | resolved | — | `core/hooks/before_task_dispatch.py` validates bound reviewer / native- |
| GAP-040 | resolved | — | `resolve-model-tier.py` returns different tiers for `--command` vs `--ag |
| GAP-041 | resolved | — | Terminal `retrospective` pause after all waves — `all_phases_merged` vs |
| GAP-042 | resolved | — | Phase `status.json` path edge case recurs — background dispatch + relati |
| GAP-043 | resolved | — | Backlog status feedback loop not mechanical — `planned` rows stay stale |
| GAP-044 | resolved | — | Stable gap IDs + index for cross-reference (manual contract): |
| GAP-045 | resolved | — | Model-dependent doc formatting breaks PRD/amendment/task automation — strict regex parsers vs free-form generation; parsers also disagree |
| GAP-046 | resolved | — | Gap absorption into a PRD/amendment does not mechanically flip status (open → planned) — upstream half of GAP-043 |
...
| GAP-048 | resolved | — | Concurrent dual-ship duplicate-PR **race** — parent + background subagent both ship same phase head → two open PRs (different bases) within minutes (variant beyond resolved GAP-035/037) |
| GAP-049 | resolved | — | `verify:failed` regression can't reach remediation — `merge-run-next` hard-exits (exit 20); only `verify:environmental` (exit 10) routes `remediate`; `conductor:no-progress` then pre-empts `remediate → /sw-stabilize` |
...
| GAP-060 | resolved | — | PRD 033 deliver: `currentWave` past `plan.waves.length` → empty batch + `await-in-flight` stall |
| GAP-061 | resolved | — | PRD 033 deliver: post-merge `verify:environmental` fixture drift (`deliver-terminal-autonomy-knob`, `deliver-resume-command-is-sw`) |
...
| GAP-074 | resolved | — | PRD 034 deliver: cross-run parallel ceiling — stale worktrees + concurrent deliver orchestrators |
