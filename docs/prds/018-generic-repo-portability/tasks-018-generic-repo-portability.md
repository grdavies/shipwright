...
- [ ] 1.1 Add root-level project-type detector (R1)
  - **File:** `scripts/detect-project-type.py`
...
- [ ] 1.3 Node allowlisted-key mapping + unsafe-command rejection (R20)
  - **File:** `scripts/detect-project-type.py`, `core/commands/sw-setup.md`
...
- [ ] 1.5 Non-interactive `--accept-defaults` path (R4)
  - **File:** `core/commands/sw-setup.md`, `scripts/seed-model-config.py` (or setup helper)
...
- [ ] 1.6 Placeholder taxonomy + `verify-unconfigured` signal + blocking semantics (R28 ← supersedes R3)
  - **File:** `scripts/verify-evidence.py`, `core/skills/verification-gate/SKILL.md`, `.sw/config.schema.json`
...
- [ ] 1.8 Portability self-check + `gh`/Actions warning (R24, R25)
  - **File:** `core/commands/sw-setup.md`, `scripts/detect-platform.py` (or setup helper)
...
- [ ] 1.9 Emit detector + presets so user installs can run setup (R12 partial)
  ...
  - **Expected:** `scripts/detect-project-type.py` ships (already emittable) and `core/sw-reference/verify-presets.json` is added to runtime-support emission; regenerate `dist/`
  - **R-IDs:** R12
...
- [ ] 1.12 Single per-repo configurator source of truth (R29)
  - **File:** `scripts/sw-configure.py` (or shared helper), `core/commands/sw-init.md`
...
- [ ] 1.13 Configured-version stamp + drift notice + non-destructive refresh (R32)
  - **File:** `core/commands/sw-init.md`, `scripts/sw-configure.py`, `.sw/config.schema.json`, shared at-entry helper
...
- [ ] 2.2 Relocate PR-test-plan manifest to `ci.*` (PRD 016-coordinated) (R10)
  - **File:** `.sw/config.schema.json`, `scripts/check-gate.py`, CI generator, PRD 016 fixtures, dogfood `.cursor/workflow.config.json`
...
- [ ] 2.3 Exclude dev-only scripts from `dist/` (R11)
  ...
  - **Expected:** `copy-to-core.sh`, `snapshot-tree.py`, `model-routing-check.py` excluded; `dist/` regenerated
  - **R-IDs:** R11
...
- [ ] 3.1 Base resolver with precedence + user-set predicate (R5)
  - **File:** `scripts/resolve-base-branch.py`
...
- [ ] 3.2 Entry guard + capture/persistence (name + SHA) (R7, R21)
  - **File:** `scripts/resolve-base-branch.py`, `scripts/shipwright-state.py`, `scripts/wave_deliver_loop.py`
...
- [ ] 3.3 Wire callers to persisted base; trunk vs integration base (R6, R26)
  - **File:** `scripts/wave_spec_seed.py`, `scripts/wave_lifecycle.py`, `scripts/wave_terminal.py`, `scripts/wave_deliver.py`, `scripts/worktree.py`
...
- [ ] 3.4 Base disclosure at entry (R22)
  - **File:** `scripts/resolve-base-branch.py`, `core/commands/sw-start.md`, `core/commands/sw-worktree.md`, deliver seed path
...
- [ ] 3.5 Fail-closed base contract for secret-scan + frozen check (R8, R19)
  - **File:** `scripts/check-frozen.py`, `scripts/secret-scan.py`, `scripts/secret_scan.py`
