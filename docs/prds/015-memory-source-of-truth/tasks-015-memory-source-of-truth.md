...
- [x] 1.1 SoT resolution helper (R1, R2)
  - **File:** `scripts/memory-sot.py`, `skills/memory/SKILL.md`
...
- [x] 2.1 Snapshot writer in freeze decision path (R4, R6, R10)
  - **File:** `core/commands/sw-freeze.md`, `scripts/memory-redact.py` (invocation)
...
- [x] 2.2 Offline-safe freeze/CI gate (R5)
  - **File:** `scripts/check-frozen.py`, `core/commands/sw-freeze.md`
  - **Expected:** freeze + `check-frozen.py` operate only on the committed snapshot, never call the provider; memory-SoT provider write is best-effort with an audit breadcrumb

### 3. Pointer inversion + supersede reconcile (M)

- [x] 3.1 Inverted authority pointer per SoT (R6)
  - **File:** `skills/memory/SKILL.md`, `scripts/memory-sot.py`
...
- [x] 3.2 Supersede manifest + reconcile (R7)
  - **File:** `docs/decisions/SUPERSEDED.log` (handling), `scripts/reconcile-status.py`, `core/commands/sw-memory-sync.md`
...
- [x] 4.3 Fail-closed redaction across writes (R10)
  - **File:** `scripts/memory-redact.py` (invocation), `skills/memory/SKILL.md`
