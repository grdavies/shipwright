...
| --- | --- | --- | --- | --- |
| `/sw-doc-review` persona panel | `skills/doc-review/SKILL.md` selection algorithm | `python3 scripts/doc-review-select.py --context-json '<signal_context>'` | `doc-review` | `migration-parity-doc-review` |
| `/sw-review` / `/sw-ship` native panel roster | `scripts/code-review-select.py` embedded rules | `python3 scripts/code-review-select.py --diff-json '<change_digest>'` | `code-review` | `migration-parity-code-review` |
| `check-gate.py` / `wave_preflight` provider configuredness | `review-local-resolve.py`, `wave_preflight.scan_review_provider` | `capability_migration_parity.select_family providers` | `providers` | `migration-parity-providers` |
| Deliver / phase entry (`wave_preflight` index freshness) | `wave_preflight.run_capability_index_check` | `capability-select.py` + freshness gate (phase 2) | `providers` | (freshness fixtures) |
...
```bash
python3 scripts/migration-parity-shadow.py --family <family> --context-json '<signal_context>'
```
...
2. Call site dispatches selector wrapper (`doc-review-select.py`, migrated `code-review-select.py`).
